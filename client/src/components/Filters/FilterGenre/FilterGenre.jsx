...
const FilterGenre = ({ setPaginate, videogames, setgames, setLoadingFilters}) => {
  ...
  const handleSelect = (event) => {
    ...
    //si tiene un db/api selected verifamos si el opcion seleccionada es diferente a allgenres en el select de géneros.
    ...
    if (select) {
      if (genre !== "allgenres") {
        ...
        //filtrar lo que devuelve genre por db o api dependiendo de la opcion selected desde  FitlerdbApi.jsx
        const dbApi = filterdbApi(genres, select.value);
        //despachamos los juegos ya filtrados por generos y db/api
        dispatch(filters(dbApi));
      }

      //en caso de selecccionar allgenres filtra todos los juegos por la option selected db/api desde FitlerdbApi.jsx
      else {
        const dbApi = filterdbApi(state.copygames, select.value);
        dispatch(filters(dbApi));
      }
    } else {
      //caso de tener un option selected desde el fitro db/api
