...
export default function FilterdbApi({
  ...
  const handleSelect = (event) => {
    ...
    //sí el filtro de db/api tiene selected allgames verifíca si en el filtro de géneros hay un option selected.
    if (target === "allgames") {
      ...
      // sí no tiene un género selected setteamos con todos los juegos sin ningun filtro
      else{
    ...
    //sí el filtro de db/api tiene selected DB o API 
      ...
      // verifíca si en el filtro de géneros hay un option selected.
      if(selectGenre){
        //filtra los juegos por el género selected desde  FilterGenre.jsx con su función de filtrado por género
        const filterByGenders = filterGenre(state.copygames,selectGenre?.value)
        //filtra los juegos de un género por la opcion selected api / db 
        const dbApi = filter(filterByGenders,target)
        setgames(dbApi);
      }
      // sí no tiene un género selected setteamos con todos los juegos solo con el filtro de db o api 
      else{
